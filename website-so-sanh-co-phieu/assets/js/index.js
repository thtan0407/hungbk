;(function ($) {
	...
	const handleRenderTable = function (data, stockType, stockCode, chartType, tablePreview, start, update = false) {
		...
		if (start === 0) {
			$(tablePreview + ' .tablePreviewTime .buttonPrev').remove();
		}
		$(tablePreview + ' .tablePreviewTime .buttonNext').prop('disabled', false)
	...
	const handleFilterCriteriaWaiting = function () {
		filterCriteriaWaiting.select2({
			...
			dropdownParent: filterCriteriaWaiting.parent(),
			minimumResultsForSearch: 20,//Infinity
			language: 'vi',
			width: '100%',
	...
	const handleFilterCriteriaVolume = function () {
		filterCriteriaVolume.select2({
			...
			dropdownParent: filterCriteriaVolume.parent(),
			minimumResultsForSearch: 20,//Infinity
			language: 'vi',
			width: '100%',
